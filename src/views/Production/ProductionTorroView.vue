...
import ProductionAddButton from '@/components/production/productionAddButton.vue'
...
import { getByNameAndSeason } from '@/api/format.js'
...
function fetch(temporada, torro) {
    getByNameAndSeason(temporada, torro).then(function (value) {
        formats.value = value
    })
}
...
function saving() {
    adding.value = false
    getByNameAndSeason(currentTemporada, currentTorro).then(function (value) {
        formats.value = value
    })
}
...
<template>
    <ProductionWrapper>
        <div class="header">
            ...
            <ProductionAddButton :active="adding" @click="addNewFormat" />
        </div>
        ...
        <div class="body">
            ...
            <Format v-for="format in formats" :title="format.nom" :data1="format.ingredients"
                :data2="[format.gramsXcaixo, format.barresXcaixo, format.barresXcaixa]" :data3="format.packagings" />
        </div>
    </ProductionWrapper>
</template>
